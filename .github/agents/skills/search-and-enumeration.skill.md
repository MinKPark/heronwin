...
- When the refreshed tree exposes a visible editable search field and the tools include `eyesandhands/set_selected_window_element_value`, prefer setting that exact field value directly over blind window-level typing.
- Use `eyesandhands/send_input_to_window` for submit keys such as `Enter` only after the query text is confirmed in the intended visible field or the direct value-entry tool is unavailable.
...
- When a visible matching result tile or row is clearly the requested target and the tools include `eyesandhands/click_selected_window_element`, prefer that exact-path click over guessed keyboard navigation if direct invocation is unavailable or unreliable.
