...
| `next` | `P1` | Finish the compact-tree evaluation rollout in `cognition`. | Use the new AVA `--evaluate-compact-tree` entry point from [Cognition Compact Tree Evaluation Rollout Plan](./designs/cognition-compact-tree-evaluation-rollout-plan.md), then run and document parity checks, benchmarks, and at least three manual evaluation passes. |
