...
internal abstract record HerfaceYamlNode;

internal sealed record HerfaceYamlScalar(string Value) : HerfaceYamlNode;

internal sealed record HerfaceYamlSequence(IReadOnlyList<HerfaceYamlNode> Items) : HerfaceYamlNode;

internal sealed record HerfaceYamlMapping(IReadOnlyDictionary<string, HerfaceYamlNode> Entries) : HerfaceYamlNode
{
    public bool TryGetValue(string key, out HerfaceYamlNode value)
    {
...
internal static class HerfaceYamlParser
{
    public static HerfaceYamlNode Parse(string text)
    {
    ...
    private static HerfaceYamlNode ParseNode(IReadOnlyList<YamlLine> lines, ref int index, int indent)
    {
        if (index >= lines.Count)
        {
    ...
    private static HerfaceYamlSequence ParseSequence(IReadOnlyList<YamlLine> lines, ref int index, int indent)
    {
        var items = new List<HerfaceYamlNode>();
        ...
        while (index < lines.Count)
        {
            ...
            {
                var entries = new Dictionary<string, HerfaceYamlNode>(StringComparer.OrdinalIgnoreCase);
                index += 1;
                ...
                items.Add(new HerfaceYamlMapping(entries));
                continue;
            }
        ...
        return new HerfaceYamlSequence(items);
    }

    private static HerfaceYamlMapping ParseMapping(IReadOnlyList<YamlLine> lines, ref int index, int indent)
        => new(ParseMappingEntries(lines, ref index, indent));

    private static IReadOnlyDictionary<string, HerfaceYamlNode> ParseMappingEntries(
    ...
    private static HerfaceYamlNode ParseNestedValueOrEmpty(
        IReadOnlyList<YamlLine> lines,
        ref int index,
        int parentIndent,
        int parentLineNumber)
    {
        ...
        {
            return new HerfaceYamlScalar(string.Empty);
        }

        return ParseNode(lines, ref index, lines[index].Indent);
    }

    private static HerfaceYamlScalar ParseScalar(string rawValue)
    {
        ...
        return new HerfaceYamlScalar(value);
    }
...
internal static class HerfaceCommandFileLoader
{
    public static IReadOnlyList<string> LoadFromFile(string path)
    {
        ...
        var root = HerfaceYamlParser.Parse(File.ReadAllText(fullPath));
    ...
    private static IEnumerable<string> ExtractCommands(HerfaceYamlNode root)
    {
        return root switch
        {
            ...
            HerfaceYamlMapping mapping when mapping.TryGetValue("commands", out var commandsNode) => ExtractCommands(commandsNode),
            _ => []
        };
    }
}
