...
internal sealed record BrainHttpClientSetup(
    HttpClient Client,
    bool BypassedBrokenLoopbackProxy,
    string? BypassedProxyValue);

internal static class BrainHttpClientFactory
{
    public static BrainHttpClientSetup Create()
    {
        var brokenProxy = FindBrokenLoopbackProxyFromEnvironment();
        if (brokenProxy is null)
        {
            return new BrainHttpClientSetup(new HttpClient(), false, null);
        }
        ...
        return new BrainHttpClientSetup(new HttpClient(handler, disposeHandler: true), true, brokenProxy);
    }
