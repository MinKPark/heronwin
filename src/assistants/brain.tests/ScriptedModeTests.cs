...
public sealed class ScriptedModeTests
{
    ...
    {
        var actual = BrainConsoleMode.ParseTars(["--help"]);
    ...
    [Fact]
    public void Parse_ReturnsScenarioPath_WhenScenarioFlagIsPresent()
    {
        var scenarioPath = Path.Combine(Path.GetTempPath(), "sample-scenario.yml");

        var actual = BrainConsoleMode.ParseTars(["--scenario", scenarioPath]);

        Assert.True(actual.IsScripted);
        Assert.Equal(Path.GetFullPath(scenarioPath), actual.ScenarioFilePath);
    }

    [Fact]
    public void Parse_Throws_WhenUnknownFlagIsUsed()
    {
        var ex = Assert.Throws<InvalidOperationException>(
            () => BrainConsoleMode.ParseTars(["--old-scripted-input", "open netflix"]));
