...
{
    public static AgentPromptCatalog Load(string assistantId = "cursor")
    {
        var currentDirectory = Directory.GetCurrentDirectory();
        var normalizedAssistantId = NormalizeAssistantId(assistantId);
        var assistantOverrideKey = $"{normalizedAssistantId.ToUpperInvariant()}_AGENT_DEFINITION_PATH";
        var configuredPath =
            Environment.GetEnvironmentVariable(assistantOverrideKey)?.Trim()
            ?? Environment.GetEnvironmentVariable("AGENT_DEFINITION_PATH")?.Trim();
        var fallbackDefinitionPath = ResolveFallbackDefinitionPath(currentDirectory, configuredPath, normalizedAssistantId);
        ...
        var coreDefinitionPaths = ResolveAssistantCoreDefinitionPaths(currentDirectory, fallbackDefinitionPath, normalizedAssistantId);
        var skillDirectories = ResolveAssistantSkillDirectories(currentDirectory, normalizedAssistantId);

        return LoadFromResolvedProfile(fallbackDefinitionPath, coreDefinitionPaths, skillDirectories);
    }
        ...
        IReadOnlyList<string> skillDirectories)
    {
        ...
        var coreSections = coreDefinitionPaths
            .Select(path => LoadPromptText(
                path,
                "agent core definition",
                warnIfMissing: false,
                stripFrontMatter: true))
        ...
        var skills = skillDirectories
            .SelectMany(LoadSkillPrompts)
            .ToList();

        return new AgentPromptCatalog(
            fallbackDefinitionPath,
            fallbackDefinition,
            coreDefinitionPaths.Count == 0 ? null : string.Join(";", coreDefinitionPaths),
            string.Join("\n\n", coreSections),
            skills);
    }

    private static string ResolveFallbackDefinitionPath(string currentDirectory, string? configuredPath, string assistantId)
    {
        if (!string.IsNullOrWhiteSpace(configuredPath))
        {
    ...
    private static IReadOnlyList<string> ResolveAssistantCoreDefinitionPaths(
        ...
        string assistantId)
    {
        ...
        var resolvedAssistantPaths = candidatePaths
            ...
            .Where(File.Exists)
            .ToArray();
        if (resolvedAssistantPaths.Length > 0)
        {
            return resolvedAssistantPaths;
        }

        var compatibilityCorePath = ResolveCoreDefinitionPath(currentDirectory, fallbackDefinitionPath);
        if (!string.IsNullOrWhiteSpace(compatibilityCorePath))
        {
            candidatePaths.Add(compatibilityCorePath);
        }
    ...
    private static IReadOnlyList<string> ResolveAssistantSkillDirectories(string currentDirectory, string assistantId)
    {
        ...
        string? frontMatterText)
    {
        ...
        try
        {
            if (BrainYamlParser.Parse(frontMatterText) is not BrainYamlMapping mapping)
            {
    ...
    private static AgentSkillActivation ParseSkillActivation(BrainYamlMapping mapping)
    {
        ...
        if (activationNode is not BrainYamlMapping activationMapping)
        {
    ...
    private static IReadOnlyList<string> ReadStringValues(BrainYamlMapping mapping, string key)
    {
        ...
        return node switch
        {
            BrainYamlScalar scalar when !string.IsNullOrWhiteSpace(scalar.Value)
                => [scalar.Value.Trim()],
    ...
    private static string? ReadOptionalScalar(BrainYamlMapping mapping, string key)
        => mapping.TryGetValue(key, out var node) && node is BrainYamlScalar scalar
            ? scalar.Value
            : null;

    private static int? ReadOptionalInt(BrainYamlMapping mapping, string key)
        => mapping.TryGetValue(key, out var node) &&
           node is BrainYamlScalar scalar &&
