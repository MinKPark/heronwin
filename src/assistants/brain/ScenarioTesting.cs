    ...
    IReadOnlyList<BrainTraceSlowEvent> SlowEvents)
{
    public string ToMarkdown()
    {
        ...
        builder.AppendLine("# HeronWin Trace Report");
        builder.AppendLine();
        builder.AppendLine($"- Trace: `{SourcePath}`");
        if (!string.IsNullOrWhiteSpace(AssistantId))
        {
            builder.AppendLine($"- Assistant: `{AssistantId}`");
        }
        ...
        {
            var completedLabel = string.IsNullOrWhiteSpace(ScenarioName) ? "Session end" : "Scenario completed";
            builder.AppendLine($"- {completedLabel}: `{scenarioCompletedAt:O}`");
        }

        var elapsedLabel = string.IsNullOrWhiteSpace(ScenarioName) ? "Session elapsed" : "Scenario elapsed";
        builder.AppendLine($"- {elapsedLabel}: `{FormatSeconds(ScenarioElapsedMs)}`");
...
internal static class BrainTraceReporter
{
    ...
    public static BrainTraceReport Generate(string path)
    {
        ...
        var assistantId = sessionStartRecord?.GetString("assistantId");
...
internal static class ScriptedConversationRunner
{
    ...
    {
        if (string.IsNullOrWhiteSpace(options.ScenarioFilePath))
        {
            Display.Error("Tars requires --scenario. Put one-step scripted work in a one-command scenario file.");
            return 1;
        }

        var suite = BrainScenarioLoader.LoadFromFile(options.ScenarioFilePath);
        ...
        Display.Info(
            $"Loaded {suite.Scenarios.Count} scenario(s) from {options.ScenarioFilePath}.");
