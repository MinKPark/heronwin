...
try
{
    consoleOptions = BrainConsoleMode.ParseCursor(args);
}
...
{
    BrainConsoleMode.PrintCursorHelp();
    return;
}
...
var config = AppConfig.Load("cursor");
...
DebugTrace.Configure(config.EnableDebugTrace || config.DebugAudioPlayback);
Display.Banner("cursor", "interactive voice/text assistant");
...
DebugTrace.WriteStructuredEvent(
    ...
    new Dictionary<string, object?>
    {
        ...
        ["assistantId"] = "cursor",
        ["launchMode"] = initialInteractiveMode.ToString().ToLowerInvariant(),
