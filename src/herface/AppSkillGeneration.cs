...
internal static class AppSkillGenerationCoordinator
{
        ...
        out AgentReply reply)
    {
        ...
        var log = $"Herface does not have a dedicated app skill group for {appName}. Say yes to generate a new `{group}` skill group first from the app's official quickstart guidance, or say no to continue opening {appName} without one.";
        ...
        out string generationUserText)
    {
        ...
        generationUserText =
            $"Generate the {offer.AppName} skill group first. Use the browser and official website or official documentation for {offer.AppName} to find the quickstart, onboarding, or first-use instructions. Then draft the new `{offer.Group}` skill group files for Herface instead of opening the app yet.";
        return true;
    }
