...
public sealed class ArtifactCleanupTests
{
    ...
    {
        var actual = ArtifactCleanup.GetLogsDirectory(@"C:\temp\brain\");

        Assert.Equal(
            Path.Combine(@"C:\temp\brain\", "logs"),
            actual);
    }
    ...
    {
        var actual = ArtifactCleanup.GetDebugVoiceRecordingDirectory(@"C:\temp\brain\");
    ...
    {
        var baseDirectory = Path.Combine(Path.GetTempPath(), "brain-artifact-tests", Guid.NewGuid().ToString("n"));
